...
get_args <- function(
  ...
  option_list <- list(
    ...
    make_option(c("--mysql_config"), type="character", default=NULL, help="path to mysql config file"))
...
mysql_config = read_yaml(opt$mysql_config)
anno = readRDS(opt$anno)

pathways = get_pathways(database_id = anno[[2]],
                        local_id = anno[[1]],
                        mysql_username = mysql_config$user,
...
# create target dataframe for targeted IDs
...
result = featureSelection_targeted(data = data,
                                     target = pheno,
