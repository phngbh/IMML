...
#suppressMessages(library(mlrMBO))
...
bed_to_df = function(
  ...
  df.char <- apply(df.char, 2, function(x) case_match(x, 1 ~ 2, 2 ~ 1, 3 ~ 0))
  rownames(df.char) <- rn
  return(df.char)
}
...
make_input_list <- function(
  ...
  id_table <- dplyr::filter(id_table, SampleID %in% names(outcome)) %>% na.omit()
  ...
  for (m in names(data_list)){
    ...
    rownames(data_list[[m]]) <- id_table$SampleID
  }
  return(data_list)
}
