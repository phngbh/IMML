...
for (i in 1:args[[2]]) {
  res <-
    read.table(
      file.path(getwd(), "magma_geneset", paste0(as.character(args[[1]]), "_", i, ".gsa.out")),
      header = T,
      sep = "",
      skip = 4
    )
...
write.table(
  data.frame(V1 = toppw),
  file.path(getwd(), paste0("toppw_", as.character(args[[1]]), ".txt")),
  col.names = F,
  row.names = F,
  quote = F
)
...
gene_df <-
  read.table(
    file.path(getwd(), "magma_gene", paste0(as.character(args[[1]]), ".genes.out")),
    header = T,
    sep = ""
  ) %>%
  arrange(ZSTAT)
...
write.table(
  ...
  file.path(getwd(), paste0("leadingEdge_", as.character(args[[1]]), ".txt")),
  col.names = F,
  row.names = F,
  quote = F
)
...
write.table(
  ...
  file.path(getwd(), paste0("leadingEdge_snps_", as.character(args[[1]]), ".txt")),
  col.names = F,
  row.names = F,
  quote = F
)
