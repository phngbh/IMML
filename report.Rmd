...
knitr::opts_knit$set(root.dir = '/home/willy/Helmholtz/testing/res_test')
library(ggplot2)
...
input_data <- readRDS("../DSPN_input_data.rds")
targets <- readRDS("../targets.rds")
modalities <- names(input_data)
annos = list()
for (mod in modalities) {
  annos[[mod]] =
    readRDS(file.path('../annotation_files', paste0('annotation_', mod, '.rds')))
}
...
```{r}
iters = 10
result_file_name = 'DSPN_FFS_glmnet_AUPRC_union_'
```

```{r}
perf_list <- list()
for (i in 1:iters){
  file <-  paste0(result_file_name, i, ".rds")
...
ggplot(filter(perf_df, Type == "AUCPR"), aes(x = Complexity, y = Value)) + 
...
```{r}
perf_df = filter(perf_df, Type == "AUCPR") %>%
...
for (i in 1:iters){
  ...
  if (file.exists(file)){
    res <- readRDS(file)
    n_max <- which.max(filter(res$perf_validate, Type == "AUCPR")$Value)
    n_modals[[i]] <- n_max
    var <- res$var[[n_max]]
    perf_test_list[[i]] <- filter(res$perf_test, Type == "AUCPR")$Value[n_max]
    var_list[[i]] <- var
  } 
}
...
for (mod in modalities) {
  ...
  var_imp_df$Name[var_imp_df$Type == mod] =
    annos[[mod]]$Name[match(var_imp_df$Var[var_imp_df$Type == mod], annos[[mod]]$Variable)]
}
